...
function Container () {
  ...
  const getKilo = (str: String) => {
    return str.replace(/brave/gi, "Kilo");
  }
  ...
  let badgeElement = (
    <BadgePrivateWindow>
      ...
      {getKilo(getLocale('headerTitle'))}
    </BadgePrivateWindow>
  )

  let dialogContent = (
    <p>
      {getKilo(getLocale('headerText1'))}
      {!isTorDisabled && (
        <>
          {' '}
          {getKilo(getLocale('headerText2'))}
        </>
      )}
    </p>
  )
  ...
  if (isWindowTor) {
    ...
    dialogContent = (
      <>
        <p>{getKilo(getLocale('headerTorText'))}</p>
      </>
    )
  }
    ...
    <BackgroundView isTor={isWindowTor}>
      ...
      <DialogBox>
        {(hasDisclaimerDismissed != null) && (
          <DisclaimerDialog
            title={isWindowTor ? getKilo(getLocale('headerTorTitle')) : getKilo(getLocale('headerTitle'))}
