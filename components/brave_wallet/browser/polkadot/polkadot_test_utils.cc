...
void PolkadotMockRpc::AddGetAccountInfo() {
  ...
  req.Set(
      ...
      base::ListValue().Append(base::ListValue().Append(base::StrCat(
          {R"(0x26AA394EEA5630E07C48AE0C9558CEF7B99D880EC681799C0CF30E8886371DA9)",
           base::HexEncode(checksum), base::HexEncode(sender_pubkey_)}))));

  if (reject_account_info_request_) {
    req_res_pairs_.emplace(req.Clone(),
                           R"(
            {
              "jsonrpc":"2.0",
              "id":8,
